...
@synthesize playButton, stopButton, showVolumeButton, hideVolumeButton,
    playButtonLand, stopButtonLand, showVolumeButtonLand, hideVolumeButtonLand;
...
{ 
    ...
    {
        kontaktView *kontakt = [[[kontaktView alloc] initWithNibName:@"kontaktView-iPad" bundle:nil] autorelease];
        [self presentModalViewController:kontakt animated:YES];
    }
    else
    {
        kontaktView *kontakt = [[[kontaktView alloc] initWithNibName:@"kontaktView-iPhone" bundle:nil] autorelease];
        [self presentModalViewController:kontakt animated:YES];
    }
}
...
- (IBAction)showVolume:(id)sender
{
    ...
    [UIView animateWithDuration:0.5
                        ...
                        options: UIViewAnimationCurveEaseOut
                     animations:^{
                         myVolumeViewLand.frame = CGRectMake(46, 124, 280, 22 );
                     } 
                     completion:^(BOOL finished){
                         NSLog(@"Done!");
                         showVolumeButtonLand.hidden = YES;
                         hideVolumeButtonLand.hidden = NO;                         
                     }];     
...
{ 
    ...
    [UIView animateWithDuration:0.5
                        ...
                        options: UIViewAnimationCurveEaseOut
                     animations:^{
                         myVolumeViewLand.frame = CGRectMake(448, 85, 5, 22 );
                     } 
                     completion:^(BOOL finished){
                         [myVolumeViewLand setHidden:YES];
                         showVolumeButtonLand.hidden = NO;
                         hideVolumeButtonLand.hidden = YES;                         
                     }];     
...
- (void)doTimer{
    ...
    if (playButton.hidden == NO)
    {
        ...
        [nazevSkladbyLabel setText:[[[NSString alloc] initWithFormat:@"%@",now] autorelease]];
        [interpretLabel setText:@""];
    }
    else
    {
